...
dd_xitong() {
		...
		dd_xitong_3() {
		  echo -e "重装后初始用户名: ${gl_huang}root${gl_bai}  初始密码: ${gl_huang}123@@@${gl_bai}Initial port:${gl_huang}22${gl_bai}"
		  echo -e "按任意键继续..."
		  read -n 1 -s -r -p ""
		  dd_xitong_bin456789
		}
...
linux_panel() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}51.  ${gl_bai}PVE开小鸡面板			 ${gl_kjlan}52.  ${gl_bai}DPanel container management panel"
	  ...
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}61.  ${gl_bai}Online translation server${gl_kjlan}62.  ${gl_bai}RAGFlow大模型知识库"
		  ...
		  9)
			...
			while true; do
				...
				if echo "quit" | timeout $timeout telnet smtp.qq.com $port | grep 'Connected'; then
				  ...
				  echo -e "${gl_hong}port$port 当前不可用${gl_bai}"
				fi
				echo ""
		  ...
		  66)
			  ...
			  case "$choice" in
				[Yy])
				  ...
				  echo -e "[${gl_lv}OK${gl_bai}] 7/10. Set time zone to${gl_huang}上海${gl_bai}"
